class contrail::quantum (
  ...
  package {
    ['openstack-quantum-contrail', 'openstack-quantum']:
      ensure => '2013.2-374';
  ...
  file {
    '/usr/bin/nodejs':
      ...
      target  => '/usr/bin/node',
      require => Package['openstack-dashboard', 'nodejs', 'python-quantumclient'],
  ...
  quantum_config {
   ...
   'DEFAULT/core_plugin':                  value => 'quantum.plugins.contrail.ContrailPlugin.ContrailPlugin';
  ...
  exec { 'contrail-dashboard':
    command  => "yum downgrade -d 0 -e 0 -y openstack-dashboard-2012.1-341.noarch",
